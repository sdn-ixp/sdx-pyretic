...
def bgp_get_best_routes(sdx, participant_name):
    ...
    # TODO: should avoid this translation and use prefixes directly for
    # performance gain.
    ...
    for peer_name in peers:
        ...
        for port in ports:
            ...
            routes = sdx.participants[participant_name].rs_client.filter_route(
                'local', 'next_hop', str(
                    port.ip))
...
def bgp_get_announced_routes(sdx, participant_name):
    ...
    routes = sdx.participants[
        participant_name].rs_client.get_all_routes('input')
