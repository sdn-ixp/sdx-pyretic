...
##
## Access methods to data structures
##
...
def step5(policy, participant_id):
    expanded_vnhop_policy = step5b_expand_policy_with_vnhop(policy, participant_id)
...
def step5b_expand_policy_with_vnhop(policy, participant_id, acc=[]):
    # Recursive call
    if isinstance(policy, parallel):
        return parallel(map(lambda p: step5b_expand_policy_with_vnhop(p, participant_id), policy.policies))
    elif isinstance(policy, sequential):
        a = []
        return sequential(map(lambda p: step5b_expand_policy_with_vnhop(p, participant_id, a), policy.policies))
    elif isinstance(policy, if_):
        return if_(step5b_expand_policy_with_vnhop(policy.pred, participant_id), step5b_expand_policy_with_vnhop(policy.t_branch, participant_id), step5b_expand_policy_with_vnhop(policy.f_branch, participant_id))
    else:
        # Base call
