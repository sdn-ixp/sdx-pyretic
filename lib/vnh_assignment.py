...
def step5c(policy, participant, participant_list,
           port_2_participant, fwd_map, VNH_2_mac):
    fwd_neighbors = set([port_2_participant[a]
                         for a in extract_all_forward_actions_from_policy(policy)])
    rewrite_policy = None
    for neighbor in fwd_neighbors:
        if neighbor != participant:
            ...
            for (a, b) in fwd_map[participant][neighbor].items():
                if not p:
                    p = (
                        if_(match(dstmac=VNH_2_mac[a]), modify(dstmac=VNH_2_mac[b]), passthrough))
                else:
                    p = p >> (
                        if_(match(dstmac=VNH_2_mac[a]), modify(dstmac=VNH_2_mac[b]), passthrough))
            if rewrite_policy and p:
                rewrite_policy += match(
                    outport=participant_list[participant][neighbor][0]) >> (p)
            else:
                if p:
                    rewrite_policy = match(
                        outport=participant_list[participant][neighbor][0]) >> (p)
...
def step5b_expand_policy_with_vnhop(policy, participant_id, sdx, acc=[]):
    # Recursive call
    if isinstance(policy, parallel):
        return parallel(map(lambda p: step5b_expand_policy_with_vnhop(
            p, participant_id, sdx), policy.policies))
    elif isinstance(policy, sequential):
        acc = []
...
def step5b(policy, participant, sdx):
    expanded_vnhop_policy = step5b_expand_policy_with_vnhop(
        policy,
        participant,
        sdx)
    ...
    if participant in sdx.participants and policy_matches is not None:
        ...
        bgp = step5b_expand_policy_with_vnhop(
            get_default_forwarding_policy(
                bgp_get_best_routes(
                    sdx,
                    participant),
                participant,
                sdx.participant_2_port),
            participant,
            sdx)
        # print bgp
...
def step4(part_2_prefix_updated, lcs, sdx):
    count = 1
    for pset in lcs:
        vname = 'VNH' + str(count)
        if vname not in sdx.VNH_2_IP:
            nhex = hex(count)
            sdx.VNH_2_IP[vname] = str(sdx.VNH_2_IP['VNH'][count])
            sdx.VNH_2_MAC[vname] = MAC(
                str(EUI(int(EUI(sdx.VNH_2_MAC['VNH'])) + count)))
        sdx.VNH_2_pfx[vname] = pset
        for participant in part_2_prefix_updated:
            # print part_2_prefix_updated
            if list(pset) in part_2_prefix_updated[
                    participant] or pset in part_2_prefix_updated[participant]:
                if participant not in sdx.part_2_VNH:
                    sdx.part_2_VNH[participant] = {}
                sdx.part_2_VNH[participant][vname] = pset
        count += 1


def step5a_expand_policy_with_prefixes(policy, participant, sdx, acc=[]):
    global participants_announcements

    # Recursive call
    if isinstance(policy, parallel):
        return parallel(map(lambda p: step5a_expand_policy_with_prefixes(
            p, participant, sdx), policy.policies))
    elif isinstance(policy, sequential):
        acc = []
...
def step5a(policy, participant, sdx, include_default_policy=False):
    p1 = step5a_expand_policy_with_prefixes(policy, participant, sdx)
    if include_default_policy:
        # TODO: this should be a + operator rather than >>
        p1 = p1 + get_default_forwarding_policy(
            bgp_get_best_routes(
                sdx,
                participant),
            participant,
...
def vnh_assignment(sdx):
    ...
    for participant in sdx.participants:
        best_routes_list = [
            x for x in bgp_get_best_routes(
                sdx,
                participant).values() if x]

        if best_routes_list:
            if (participant in participant_2_prefix):
                participant_2_prefix[participant] = participant_2_prefix[
                    participant] + best_routes_list
            else:
    ...
    for participant in participant_2_prefix:
        sdx.part_2_prefix_old[participant] = tuple(
            participant_2_prefix[participant])
    ...
    if (participant_2_prefix):
        ...
        # Step 4: Assign VNHs
        step4(part_2_prefix_updated, lcs, sdx)
...
def pre_VNH(policy, sdx, participant_name, participant):
    if isinstance(policy, parallel):
        return parallel(
            map(lambda p: pre_VNH(p, sdx, participant_name, participant), policy.policies))
    elif isinstance(policy, sequential):
        return sequential(
            map(lambda p: pre_VNH(p, sdx, participant_name, participant), policy.policies))
    elif isinstance(policy, if_):
        ...
        if isinstance(policy, modify):
            ...
            if 'state' in policy.map:
                ...
                if 'in' in state:
                    ...
                    # return
                    # fwd(sdx.participant_2_port[participant_name][participant_name][int(pn)])
...
def post_VNH(policy, sdx, participant_name):
    ...
    for port in port_2_state:
        if 'out' in port_2_state[port]:
            pol = pol + \
                (match(outport=port) >> modify(
                    state=port_2_state[port], dstmac=port_2_mac[port]))
        else:
            pol = pol + \
                (match(outport=port) >> modify(state=port_2_state[port]))
    return policy >> pol
