...
class SDX(object):
    ...
    def add_participant(self, participant, name):
        ...
        self.participant_id_to_in_var[
            participant.id_] = "in" + participant.id_.upper()
        i = 0
        for port in participant.phys_ports:
            self.port_id_to_out_var[
                port.id_] = "out" + participant.id_.upper() + "_" + str(i)
            self.out_var_to_port[
                "out" +
                participant.id_.upper() +
                "_" +
                str(i)] = port
            i += 1
    ...
    def fwd(self, port):
        if isinstance(port, PhysicalPort):
            return modify(
                state=self.port_id_to_out_var[port.id_], dstmac=port.mac)
        else:
            return modify(
                state=self.participant_id_to_in_var[port.participant.id_])
###
...
def sdx_from(vport):
    ...
    for phys_port in vport.participant.phys_ports:
        match_all_physical_port = match_all_physical_port | match(
            srcmac=phys_port.mac)
...
def sdx_participant_policies(sdx_config):
    ...
    for k in [0, 1]:
        ...
        sdx_policy = sequential([
            sdx_policy,
            parallel(
                [sdx_restrict_state(sdx_config, sdx_config.participants[
                    participant]) for participant in sdx_config.participants]
            )])
    return sdx_policy
...
def sdx_parse_config(config_file):
    ...
    for participant_name in sdx_config:
        ...
        for i in range(0, len(participant["Ports"])):
            sdx.port_2_participant[
                participant["Ports"][i]['Id']] = participant_name
            sdx.participant_2_port[participant_name][
                participant_name].append(participant["Ports"][i]['Id'])
        # print sdx_ports[participant_name]
        ''' Adding virtual port '''
        print "Adding virtual ports for ", participant_name
        sdx_vports[participant_name] = VirtualPort(
            participant=participant_name,
            id_=participant["Ports"][0]['Id'])  # Check if we need to add a MAC here
    ...
    for participant_name in sdx_config:
        ...
        for peer_name in sdx_config[participant_name]["Peers"]:
            ...
            sdx.participant_2_port[participant_name][
                peer_name] = [sdx_vports[peer_name].id_]
...
def sdx_parse_policies(policy_file, sdx):
    ...
    for participant_name in sdx.participants:
        ...
        classifier.append(
            sdx.participants[participant_name].policies.compile())
...
def sdx_parse_announcements(announcement_file, sdx):
    ...
    for participant_name in sdx_announcements:
        ...
        for announcement_module in announcement_modules:
            participant.custom_routes.extend(
                announcement_module.custom_routes(
                    participant,
                    sdx))
